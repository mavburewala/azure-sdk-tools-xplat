    ...
    computeManagementClient.hostedServices.get(dnsPrefix, function (error, response) {
      ...
      if (error) {
        ...
        computeManagementClient.deployments.getBySlot(dnsPrefix, 'Production', function (error, response) {
            if(options.virtualNetworkName){
                if(!response.virtualNetworkName||response.virtualNetworkName!=options.virtualNetworkName){
                    return cmdCallbackHook(new Error($('Previous deployment does not have any virtualNetowrk')));
                }
                else if(response.virtualNetworkName!=options.virtualNetworkName)
                {
                    return cmdCallbackHook(new Error($('Previous deployment has  different virtualNetowrk ('+response.virtualNetworkName+') as specified .')));
                }
            }
            else
            {
                if(response.virtualNetworkName||response.virtualNetworkName!=null||response.virtualNetworkName!=""){
                    return cmdCallbackHook(new Error($('Previous deployment has virtualNetowrk but currently it is not specified')));
                }
            }
